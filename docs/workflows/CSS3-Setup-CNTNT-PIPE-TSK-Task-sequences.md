...
<p>This workflow reacts to the completion of CSS implementation/setup tasks (there is an overlap between the two processes). In some cases, completing a task leads to the creation of the next task in the sequence; in others it causes a property change (e.g. <em>Content</em> <u>In progress</u> → <u>In review</u>) or a pipeline movement (<u>Set-up</u> → <u>Ready for launching</u>).</p><p>For <u>Chase POC</u>, the workflow triggers an email to be sent (if appropriate).</p><p>As this is a task workflow, the <em>HSTrigger</em> property is used to trigger ticket workflows where necessary for update.</p></details>
...
3. Forces ticket updates.
4. Determines <u>Chase POC</u> status and action.  
5. Set contacts and initial *Comms stage* – Comms workflows will not operate if this property is blank.  
